...
void Ivy::setSet(float aa, float bb, float cc, float dd, float ee, float ff, float gg, float hh, float ii, float jj, float kk)
{
	primaryWeight = bb;
...
void Ivy::multiple_seed(Vector3d seedPos)
{
	

		IvyNode tmpNode;

		tmpNode.pos = Vector3d(seedPos.x, seedPos.y, seedPos.z);
...
/*
void Ivy::multiple_seed(Vector3d2* seedPos)
{
	while (seedPos->next != nullptr)
	{
...
Vector3d Ivy::computeAdhesion(const Vector3d &pos)
{
	...
	std::vector<BasicTriangle> triangless = Common::mesh.triangles;
	//find nearest triangle
	for (std::vector<BasicTriangle>::iterator t = triangless.begin(); t != triangless.end(); ++t)
	{
